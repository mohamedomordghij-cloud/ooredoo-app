...
import { sendAlertSms } from "./smsNotifier.js";
...
function getLevelAndThreshold(metricName, value, baseline) {
  ...
  if (metricName === "gasLevel") {
    if (value > 500) return { severity: "critical", thresholdExceeded: 500 };
    if (value >= 300) return { severity: "warning", thresholdExceeded: 300 };
    return { severity: "normal" };
  }

  // BMP280 Pressure
  if (metricName === "pressure") {
    if (value < 970 || value > 1050) return { severity: "critical", thresholdExceeded: value < 970 ? 970 : 1050 };
    if ((value >= 970 && value < 990) || (value > 1030 && value <= 1050)) return { severity: "warning", thresholdExceeded: value > 1030 ? 1030 : 990 };
    return { severity: "normal" };
  }
...
export async function processMetric({ nodeId, zoneId, metricName, value, baseline, nodeName, zoneName, datacenterName }) {
...
if (severity === "critical") {
  sendAlertSms({
    ...
    metricName,
    metricValue: value,
    nodeName: nodeName ?? String(nodeId),
    zoneName: zoneName ?? String(zoneId),
    datacenterName: datacenterName ?? "Datacenter",
